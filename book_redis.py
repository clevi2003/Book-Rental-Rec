...
class BookAPI:
    ...
    def add_rating(self, file_name):
        ...
        with open(file_name, "r") as file:
            ...
            while True:
                ...
                if len(rating) < 2:
                    break
                # adds user to list of users who have rated the book
                self.r.hset(rating[1], mapping={rating[0]: rating[2]})
    ...
    def rent_book(self, user, isbn):
        ...
        self.r.hset(f'{user}:{isbn}',
                    mapping={'rentDate': today, 'returnDate': week})

    def get_users_ratings(self, user):
        # gets list of user's ratings
        users_ratings = self.r.hgetall(user)
        return users_ratings

    def get_users_who_rated(self, isbn):
        # gets list of users who rated a given book and their rating
        users_who_rated = self.r.hgetall(isbn)
